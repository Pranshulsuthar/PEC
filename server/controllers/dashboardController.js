...
exports.student = async (req, res) => {
  try {
    ...
    const [mentor] = await pool.query(`SELECT u.id, u.name, u.email, mp.mentor_code, mp.group_id, mp.group_name, mp.specialization, mp.department FROM mentor_student ms JOIN users u ON u.id = ms.mentor_id LEFT JOIN mentor_profiles mp ON mp.user_id = u.id WHERE ms.student_id = ? AND ms.status = 'active' LIMIT 1`, [userId]);
...
exports.mentor = async (req, res) => {
  try {
    ...
    const [students] = await pool.query(`SELECT u.id, u.name, u.email, sp.student_code, sp.branch, sp.year, sp.roll_number, mp.group_id, mp.group_name, COUNT(DISTINCT ta.task_id) AS assigned_tasks, COUNT(DISTINCT CASE WHEN ts.status = 'accepted' THEN ts.task_id END) AS completed_tasks, COALESCE(SUM(CASE WHEN ts.status = 'accepted' THEN ts.score ELSE 0 END), 0) AS points FROM mentor_student ms JOIN users u ON u.id = ms.student_id LEFT JOIN student_profiles sp ON sp.user_id = u.id LEFT JOIN mentor_profiles mp ON mp.user_id = ms.mentor_id LEFT JOIN task_assignments ta ON ta.student_id = u.id LEFT JOIN task_submissions ts ON ts.student_id = u.id GROUP BY u.id, u.name, u.email, sp.student_code, sp.branch, sp.year, sp.roll_number, mp.group_id, mp.group_name ORDER BY u.name`, [userId]);
    ...
    const [notifications] = await pool.query('SELECT * FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT 6', [userId]);
    res.json({ success: true, profile, students, capacity: { assigned: students.length, maximum: 4, available: Math.max(0, 4 - students.length), full: students.length >= 4 }, submissions, pendingReviews: pendingReviews.count, tasks, notifications, news: [], events: [] });
  } catch (err) { console.error(err); res.status(500).json({ success: false, message: 'Server error' }); }
};
...
exports.coordinator = async (req, res) => {
  try {
    ...
    res.json({ success: true, stats: { students: Number(students.count), activeStudents: Number(activeStudents.count), mentors: Number(mentors.count), tasks: Number(tasks.count), solved: Number(solved.count), events: Number(events.count) }, recentStudents, notifications, news: allNews, events: await getEvents() });
  } catch (err) { console.error(err); res.status(500).json({ success: false, message: 'Server error' }); }
};
